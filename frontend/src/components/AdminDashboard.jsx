...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      const { data } = await API.get(`/${activeSection}`);
      setDataList(data);
    } catch (error) {
      console.error("❌ Error fetching data:", error);
      setMsg("Failed to fetch data. Check server.");
      setDataList([]);
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      await API.post(`/${activeSection}`, payload);
      setMsg(`${activeSection.slice(0, -1)} added successfully!`);
      setFormData({});
      fetchData();
    } catch (error) {
      console.error("❌ Error adding item:", error);
      setMsg("Failed to add item. Check console.");
    }
  };
  ...
  const renderForm = () => {
    switch (activeSection) {
      case "flights":
        return (
          <>
            <input placeholder="Airline" value={formData.airline || ""} onChange={(e) => setFormData({ ...formData, airline: e.target.value })} />
            <input placeholder="From" value={formData.from || ""} onChange={(e) => setFormData({ ...formData, from: e.target.value })} />
            <input placeholder="To" value={formData.to || ""} onChange={(e) => setFormData({ ...formData, to: e.target.value })} />
            <input type="number" placeholder="Price" value={formData.price || ""} onChange={(e) => setFormData({ ...formData, price: e.target.value })} />
            <input placeholder="Departure" value={formData.departure || ""} onChange={(e) => setFormData({ ...formData, departure: e.target.value })} />
            <input placeholder="Arrival" value={formData.arrival || ""} onChange={(e) => setFormData({ ...formData, arrival: e.target.value })} />
      ...
      case "hotels":
        return (
          <>
            <input placeholder="Hotel Name" value={formData.name || ""} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
            <input placeholder="Location" value={formData.location || ""} onChange={(e) => setFormData({ ...formData, location: e.target.value })} />
            <input type="number" placeholder="Price per Night" value={formData.pricePerNight || ""} onChange={(e) => setFormData({ ...formData, pricePerNight: e.target.value })} />
            <input placeholder="Image URL" value={formData.image || ""} onChange={(e) => setFormData({ ...formData, image: e.target.value })} />
      ...
      case "packages":
        return (
          <>
            <input placeholder="Package Name" value={formData.name || ""} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
            <input type="number" placeholder="Price" value={formData.price || ""} onChange={(e) => setFormData({ ...formData, price: e.target.value })} />
            <input placeholder="Image URL" value={formData.image || ""} onChange={(e) => setFormData({ ...formData, image: e.target.value })} />
      ...
      case "destinations":
        return (
          <>
            <input placeholder="Destination Name" value={formData.name || ""} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
            <input placeholder="Image URL" value={formData.image || ""} onChange={(e) => setFormData({ ...formData, image: e.target.value })} />
          </>
        );
